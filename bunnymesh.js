...
class goPiece{
  constructor(player,coord){
    console.log('goPiece class, constructors: piece added to board by',player==GO_PIECE_WHITE?'GO_PIECE_WHITE':'GO_PIECE_BLACK','at',coord);
    this.player=player;
  ...
  exclude(checkPlayer){
    return (typeof checkPlayer!='undefined' && checkPlayer.indexOf(this.order)!=-1);
  }
//1
  hasFreedom(checkPlayer){
    ...
    if(typeof checkPlayer!=='undefined'){
      ...
      checkPlayer.push(this.order);
    }
    else {
      console.log('-','root query');
      checkPlayer=[this.order];
    }
    ...
    if(
      (this.coord.x<1 || goPiece.cannotEscape(leftCell,this.player,checkPlayer)) &&
      (this.coord.x>17 || goPiece.cannotEscape(rightCell,this.player,checkPlayer)) &&
      (this.coord.y<1 || goPiece.cannotEscape(topCell,this.player,checkPlayer)) &&
      (this.coord.y>17 || goPiece.cannotEscape(bottomCell,this.player,checkPlayer))
    )
      return false;
  return true;
  }
  ...
  static cannotEscape([x,y],player,checkPlayer)
  {
    ...
    return goPiece.isOccupied(x,y) && (goBoard[x][y].exclude(checkPlayer) || !goBoard[x][y].freeAndFriend(player,checkPlayer));
  }
//2
